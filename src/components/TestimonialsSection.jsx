...
const TestimonialsSection = () => {
    ...
    return (
        <section id="testimonials" className="py-32 bg-brand-cream relative overflow-hidden">
            {/* Creative Background */}
            ...
            <div className="container mx-auto px-6 relative z-10">
                <div className="text-center mb-4 md:mb-8 relative">
                    ...
                    <h2 className="text-5xl md:text-7xl text-brand-dark font-abril leading-none mb-4 md:mb-8">
                        Loved By <br />
                        <span className="relative inline-block">
                            <span className="relative z-10">Thousands</span>
                            <svg className="absolute bottom-2 left-0 w-full h-3 text-brand-accent -z-10 opacity-40" viewBox="0 0 100 10" preserveAspectRatio="none">
                    ...
                    <div className="flex justify-center gap-4 mt-4 md:mt-6">
                        <button
                            onClick={() => scroll('left')}
                            className="w-12 h-12 rounded-full border-2 border-brand-dark flex items-center justify-center hover:bg-brand-dark hover:text-brand-cream transition-all duration-300 group"
                    ...
                    <div ref={scrollContainerRef} className="flex overflow-x-auto py-12 px-4 gap-8 snap-x snap-mandatory hide-scrollbar">
                        {testimonials.map((t, i) => (
                            <div key={i} className="min-w-full md:min-w-[calc(50%-16px)] lg:min-w-[calc(33.333%-22px)] snap-center">
                                <div className="bg-white p-10 rounded-[3rem] relative group hover:-translate-y-2 transition-transform duration-500 shadow-xl hover:shadow-2xl border border-brand-dark/5 h-full flex flex-col">
                                    <Quote className="w-12 h-12 text-brand-accent mb-6 transform group-hover:scale-110 transition-transform" />

                                    <p className="text-xl text-brand-dark leading-relaxed mb-8 font-medium flex-grow italic">
                                        "{t.text}"
                                    </p>

                                    <div className="flex items-center gap-4 mt-auto pt-6 border-t border-brand-dark/5">
                                        <div className="relative">
                                            <img src={t.img} className="w-16 h-16 rounded-full object-cover border-2 border-brand-accent p-1" alt={t.name} />
                                            <div className="absolute -bottom-1 -right-1 bg-brand-accent text-white text-[10px] font-bold px-2 py-0.5 rounded-full">
                                                5.0 ★
                                            </div>
                                        </div>
                                        <div>
                                            <h4 className="font-bold text-brand-dark text-lg font-abril tracking-wide">{t.name}</h4>
                                            <p className="text-sm text-brand-accent font-bold tracking-wider uppercase">{t.role}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="min-w-full md:min-w-[calc(50%-16px)] lg:min-w-[calc(33.333%-22px)] snap-center flex items-center justify-center">
                            <div className="bg-brand-dark p-10 rounded-[3rem] text-center text-brand-cream flex flex-col items-center justify-center h-full w-full shadow-xl transform hover:scale-[1.02] transition-transform cursor-pointer group">
                                <div className="w-20 h-20 bg-brand-accent rounded-full flex items-center justify-center mb-6 group-hover:rotate-12 transition-transform">
                                    <Star className="w-10 h-10 text-white fill-current" />
                                </div>
                                <h3 className="text-3xl font-abril mb-4">Join the Family</h3>
                                <p className="text-brand-cream/70 mb-8 max-w-xs">Share your experience and get featured on our wall of love.</p>
                                <button
                                    onClick={() => setIsReviewModalOpen(true)}
                                    className="bg-brand-cream text-brand-dark px-8 py-3 rounded-full font-bold hover:bg-white transition-colors"
                                >
                                    Leave a Review
                                </button>
                            </div>
